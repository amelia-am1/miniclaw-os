...
/**
 * Ensure external collaborators have triage permission only.
 * The owner retains admin. Everyone else gets triage.
 */
async function enforceCollaboratorPermissions(
  ...
  try {
    ...
    for (const collab of collaborators) {
      ...
      // If already triage or less, skip
      if (collab.role_name === "triage" || collab.role_name === "read") continue;

      try {
        ghApi("PUT", `/repos/${repo}/collaborators/${collab.login}`, {
          permission: "triage",
        });
        logger.info(`Set ${collab.login} to triage permission on ${repo}`);
      } catch (err: unknown) {
        const e = err as { stderr?: string; message?: string };
        logger.warn(`Failed to set triage for ${collab.login}: ${e.stderr || e.message || "unknown"}`);
      }
    }
  } catch (err: unknown) {
    const e = err as { stderr?: string; message?: string };
