...
export function registerHumanCommands(ctx: CliContext, cfg: HumanConfig): void {
  ...
  cmd
    ...
    .option("--interactive", "Request interactive mode (ask_human) for sudo operations", false)
    .action(async (opts: Record<string, string | boolean>) => {
      ...
      if (interactive) {
        console.log("Running in interactive mode (ask_human will handle sudo prompts)");
      }
      ...
      try {
        ...
        if (result.success) {
          ...
          if (hasSudoErrors && !interactive) {
            console.log("⚠  Some steps require sudo. Try running with --interactive flag:");
            console.log("   openclaw mc-human setup --interactive");
          } else {
  ...
  // Verify subcommand (smoke test helper)
  cmd
    .command("verify")
    ...
    .action(async () => {
      ...
      try {
        ...
        console.log(`  ${results.vnc_reachable ? "✓" : "✗"} VNC reachable at 127.0.0.1:5900`);
        console.log(`  ${results.system_sleep_disabled ? "✓" : "✗"} System sleep disabled (pmset sleep=0)`);
        console.log(`  ${results.display_sleep_configured ? "✓" : "✗"} Display sleep configured`);
        console.log(`  ${results.screensaver_disabled ? "✓" : "✗"} Login window screensaver disabled`);
        console.log("");
