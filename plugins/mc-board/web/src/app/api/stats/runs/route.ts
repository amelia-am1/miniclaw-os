...
const EMPTY = { total_runs: 0, total_tokens: 0, avg_tokens_per_card: 0, avg_duration_ms: 0 };
  ...
  try {
    ...
    const row = db.prepare(`
      SELECT
        ...
        COALESCE(AVG(peak_tokens), 0) as avg_tokens_per_card,
        COALESCE(AVG(duration_ms), 0) as avg_duration_ms
      FROM agent_runs
    `).get() as { total_runs: number; total_tokens: number; avg_tokens_per_card: number; avg_duration_ms: number };
    db.close();
