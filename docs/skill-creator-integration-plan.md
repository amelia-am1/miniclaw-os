...
The existing `/api/process/[column]/[cardId]` route spawns Claude with a prompt loaded from `~/.openclaw/user/<bot_id>/brain/prompts/in-progress-process.txt`.
...
- Edit `~/.openclaw/user/<bot_id>/brain/prompts/in-progress-process.txt` to detect the `skill` tag
...
**Files to edit:**
- `~/.openclaw/user/<bot_id>/brain/prompts/in-progress-process.txt`
...
| in-progress process prompt | `~/.openclaw/user/<bot_id>/brain/prompts/in-progress-process.txt` |
