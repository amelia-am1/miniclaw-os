...
|----------|---------|---------|
| `MINICLAW_VERSION` | `v0.1.2` | Tag to clone when installing miniclaw-os repo |
| `OPENCLAW_STATE_DIR` | `$HOME/.openclaw` | MiniClaw / OpenClaw state dir (cards, logs, workspace, config). Override to use a custom location. |
| `OPENCLAW_DIR` | `$OPENCLAW_STATE_DIR` | OpenClaw home (config, state root). Set automatically from `OPENCLAW_STATE_DIR`. |
| `LOCAL_BIN` | `$HOME/.local/bin` | Where CLI tools are copied |

Set these before running if you want a non-default location:

```bash
OPENCLAW_STATE_DIR="$HOME/my-openclaw" ./install.sh
```
...
- Creates `$OPENCLAW_STATE_DIR/openclaw.json` with minimal defaults if it does not
  exist.
...
- If the repo is already cloned at `$OPENCLAW_STATE_DIR/projects/miniclaw-os` at the
  right version tag: skips.
- If the repo exists but is at a **different** version tag: runs
  `rm -rf $OPENCLAW_STATE_DIR/projects/miniclaw-os` then re-clones at the requested
...
Creates:
- `$OPENCLAW_STATE_DIR/miniclaw/plugins/`
- `$OPENCLAW_STATE_DIR/projects/`

### Step 6 — Plugins

For each plugin directory under `plugins/` in the repo:
- Copies it to `$OPENCLAW_STATE_DIR/miniclaw/plugins/<name>/` using `rsync`
...
`$OPENCLAW_STATE_DIR/miniclaw/plugins/` but absent from `openclaw.json`. They will
...
Creates:
- `$OPENCLAW_STATE_DIR/user/memory/`
- `$OPENCLAW_STATE_DIR/soul-backups/`

### Step 10 — QMD collections

Registers a `mc-memory` QMD collection pointing at `$OPENCLAW_STATE_DIR/user/memory/`.
Non-fatal if QMD is not installed yet.

### Step 11 — Vault

- Initialises the vault at `$OPENCLAW_STATE_DIR/miniclaw/SYSTEM/vault/` if
...
├── .openclaw/                 # OpenClaw home (default; override with OPENCLAW_STATE_DIR)
...
The `mc-*` plugins are being tested by hand, but Am builds them faster than we
can verify. If you run into an issue with any plugin beyond the base install
(like mc-square, mc-stripe, etc.), tell your "Am" to debug and fix the tool —
she knows how to fix herself or contact us for support when needed.
...
# custom:  ~/my-openclaw/logs/miniclaw-board-web.log (if OPENCLAW_STATE_DIR=$HOME/my-openclaw)
```
